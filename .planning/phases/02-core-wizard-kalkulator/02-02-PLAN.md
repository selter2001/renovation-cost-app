...
<task type="auto">
  <name>Task 1: WizardPage, progress bar, navigation, animated transitions, route and CTA link</name>
  <files>src/pages/WizardPage.tsx, src/components/wizard/WizardProgress.tsx, src/components/wizard/WizardNav.tsx, src/router/index.tsx, src/pages/HomePage.tsx</files>
  <action>
**src/pages/WizardPage.tsx** -- Kontroler krokow wizarda:

Import { AnimatePresence, motion } from 'motion/react'. Import useWizardStore (currentStep, nextStep, prevStep). Import step components: RoomsStep, DimensionsStep. For steps 2 and 3 (WorksStep, SummaryStep) render placeholder divs with text "WorksStep" / "SummaryStep" -- te zostaną zbudowane w planie 02-03.
...
Add route under Layout children: `{ path: "wizard", element: <Suspense fallback={<div>...</div>}><WizardPage /></Suspense> }`.
...
UWAGA: uzywaj HashRouter -- Link to="/wizard" przejdzie do /#/wizard.
  ...
  <verify>
`pnpm build` przechodzi. Route /wizard renderuje WizardPage. Przycisk CTA na HomePage prowadzi do /wizard. Kliknięcie Dalej/Wstecz zmienia kroki z animacja. Pasek postepu aktualizuje sie.
  </verify>
  <done>WizardPage z 4 krokami renderuje sie pod /wizard. Progress bar pokazuje aktualny krok. Nawigacja Wstecz/Dalej dziala z animowanymi przejsciami. CTA na HomePage prowadzi do /wizard. Kroki 2-3 sa placeholderami.</done>
</task>
...
Import ROOM_TYPES from @/types/wizard. Import useWizardStore (rooms, addRoom, removeRoom, updateRoomName). Import useTranslation. Import RoomCard. Import Card from shadcn.

Layout:
1. Top section: heading t('rooms.selectType') with grid of 7 room type cards (ROOM_TYPES). Grid: grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3. Each card shows Lucide icon + translated label. On click: addRoom(type, t(labelKey)). Use Lucide icons dynamically -- import all needed icons: CookingPot, Bath, Sofa, Bed, DoorOpen, Monitor, LayoutGrid from 'lucide-react'. Create a map from icon name string to component: `const ICON_MAP: Record<string, LucideIcon> = { CookingPot, Bath, Sofa, Bed, DoorOpen, Monitor, LayoutGrid }`. Render icon from map.
